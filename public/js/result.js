...
var answers=document.getElementById('correct').innerText.split(".");

var correctAnswer = [answers[1], answers[2], answers[3], answers[4], answers[5]];
...
// to count the correct answer and get the result
  ...
  for (i = 0; i < 5; i++) {
    if ((correctAnswerArray[i].substring((correctAnswerArray[i].indexOf('.')+2))) === sessionStorage.getItem(`question${i + 1}`).trim())
      counter++;
  }
